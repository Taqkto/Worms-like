...
class StartMenu:
    ...
    def __init__(self, width: int, height: int, font: pygame.font.Font, player_count: int = 2):
        ...
        # Nouveau: permettre de choisir le nombre de joueurs
        self.min_players = 2
        self.max_players = 8
        self.player_count = max(self.min_players, min(player_count, self.max_players))

        self.stage = 0  # 0: choix joueurs, 1: choix personnages, 2: nommage, 3: carte
        self.num_chars = 1
        self.max_chars = 4
    ...
    def handle_event(self, event):
        ...
        if self.stage == 0:  # Sélection du nombre de joueurs
            if event.key == pygame.K_LEFT:
                self.player_count = max(self.min_players, self.player_count - 1)
                self.rebuild_names()
        ...
        elif self.stage == 1:  # Sélection du nombre de personnages
            if event.key == pygame.K_LEFT:
        ...
        elif self.stage == 2:  # Nommage des personnages
        ...
        elif self.stage == 3:  # Sélection de la carte
            if event.key == pygame.K_LEFT:
    ...
    def draw(self, surface: pygame.Surface):
        ...
        y = 20
        if self.stage == 0:
            draw_text("Choose number of players: (LEFT / RIGHT)  ENTER to confirm", y)
            y += 40
            draw_text(f"Players: {self.player_count}", y, (200, 200, 120))
